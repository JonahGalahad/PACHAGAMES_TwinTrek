...
public class Player2 : MonoBehaviour
{
    ...
    public bool compaAtrapado = false;
    private Collider2D detectarASam; //Variable que detecta la colision del objeto hijo del jugador
    ...
    void Start()
    {
        detectarASam = transform.Find("DetectarSam").GetComponent<Collider2D>();
    }
    ...
    public void LiberarCompa() //Metodo para la mecanica de Atrapar de la FLOR
    {
        ...
        {
            sam.gameObject.GetComponent<Player1>().DejarEstarAtrapado();
            detectarASam.enabled = true;
            compaAtrapado = false;
            zonaLiberar = false;
            paraLiberar = true;
    ...
    {
        if (collision.gameObject.CompareTag("Sam"))
        {
            if (compaAtrapado) //Metodo para la mecanica de Atrapar de la FLOR
            {
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Sam"))
        {
            if (collision.gameObject.GetComponent<Player1>().atrapado == true && compaAtrapado == false) //Metodo para la mecanica de Atrapar de la FLOR
            {
                compaAtrapado = true;
                detectarASam.enabled = false;
            }
            else if(collision.gameObject.GetComponent<Player1>().atrapado == false && compaAtrapado == true)
            {
                compaAtrapado = false;
                detectarASam.enabled = true;
            }
